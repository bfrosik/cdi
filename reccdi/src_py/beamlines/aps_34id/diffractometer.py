...
basically an information agglomerator for the viz generation.
    ...
    def __init__(self, config):
        ...
        try:
            ...
            delta, gamma, theta, phi, chi, scanmot, scanmot_del, detdist, detector, energy = sput.parse_spec(specfile,
                                                                                                             last_scan)
            self.delta = delta
            self.gamma = gamma
            self.theta = theta
...
class CXDViz():
    ...
    def set_geometry(self, p, shape):
        ...
        # could probably put the diffractometer specification into the config file as well.
        # the y+ z- x- y+ x- describes 34idc.  Could even make a diffractometer class that defines this.
