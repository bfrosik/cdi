...
cdef extern from "../include/bridge.hpp":
    cdef cppclass Bridge:
        Bridge() except +
        void StartCalcWithGuess(vector[float], vector[float], vector[float], vector[int], string, int)
        void StartCalc(vector[float], vector[int], string, int)
        void StartCalcMultiple(vector[float], vector[int], string, int, int)
...
cdef class PyBridge:
    ...
    def __dealloc__(self):
        del self.thisptr
    def start_calc_with_guess(self, data_r, guess_r, guess_i, dims, config, stage):
        self.thisptr.StartCalcWithGuess(data_r, guess_r, guess_i, dims, config.encode(), stage)
    def start_calc(self, data_r, dims, config, stage):
        self.thisptr.StartCalc(data_r, dims, config.encode(), stage)
    def start_calc_multiple(self, data_r, dims, config, no_threads, stage):
        self.thisptr.StartCalcMultiple(data_r, dims, config, no_threads, stage)
