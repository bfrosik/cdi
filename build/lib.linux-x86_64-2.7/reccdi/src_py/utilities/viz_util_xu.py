...
read from config file on
    ...
    def __init__(self, config):
        ...
        try:
            ...
            self.lamda, delta, gamma, dth, arm, pixel,energy = sput.parse_spec(specfile,
last_scan)
...
class CXDViz():
  ...
  def set_geometry(self, params, shape):
    ...
    print("running the xrayutilities version")
    ...
    q1=np.array(qc.area(0.0,0,0,tth,gam,deg=False))
    q2=np.array(qc.area(dth,0,0,tth,gam,deg=False))
    Astar=q1[:,1,0]-q1[:,0,0]
    Bstar=q1[:,0,1]-q1[:,0,0]
    Cstar=(q2-q1)[:,0,0]
